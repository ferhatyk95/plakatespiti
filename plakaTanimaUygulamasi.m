function plakaTanimaUygulamasi
    % Uygulama figürü ve bileşenlerin oluşturulması
    f = figure('Visible','off','Position',[360,500,450,285],'Color',[0.5 0.8 0.5]); % Arka plan rengini yeşil yap
    movegui(f, 'center'); % Figürü ekranın ortasına taşı

    % Başlık ekle
    titleText = uicontrol('Style','text','String','Plaka Tanıma Uygulaması',...
        'Position',[135,250,180,30],'FontSize',14,'FontWeight','bold','BackgroundColor',[0.5 0.8 0.5]);

    % Resim seçme düğmesi
    selectButton = uicontrol('Style','pushbutton','String','Resim Seç',...
        'Position',[315,180,70,25],...
        'Callback',@selectButton_Callback,...
        'BackgroundColor',[0.2 0.6 1],... % Düğme rengini mavi tonunda yap
        'ForegroundColor',[1 1 1]); % Yazı rengini beyaz yap

    % Sonuç metni kutusu
    resultText = uicontrol('Style','text','Position',[135,50,180,25],'BackgroundColor',[1 1 1]); % Metin kutusu arka plan rengini beyaz yap

    % Seçilen resmin görüntüsü
    axesHandle = axes('Units','Pixels','Position',[50,80,200,150]);
    ...
    function selectButton_Callback(source, event)
        ...
        if ~isequal(fileName, 0)
            ...
            % Plaka tanıma işlemi
            [recognizedText, processedImage] = plakaTanimla(I);
...
function [recognizedText, processedImage] = plakaTanimla(I)
